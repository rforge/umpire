...
requested. It will do so using 10 "components", where eadch component consists
...
For categorical feature, the user may tune the percent of categorical 
data desired to be nominal and the range of number of categories to be 
simulated. 
...
rows are treated as features and columns as patients. The <tt>setDataTypes</tt>
method transposes its results to a data frame where the columns are features
and the rows are patients. This transpoition both fits better with the
conventions used for clinical data, but also supports the ability to store
different kinds of (mixe-type) data in different columns
...
<tt>setDataTypes</tt> generates a list containing two objects:
...
The <tt>cutpoints</tt> (a record, for each feature, of data 
type, break points, and labels) can be saved for downstream use in 
...
3. The <tt>cutpoints</tt> generated by <tt>setDataTypes</tt>.

```{r}
mte <- MixedTypeEngine(ce,
                       noise = cnm,
                       cutpoints = dt$cutpoints)
summary(mte) # should add a customized summary method for MixedTypeEngine
```
